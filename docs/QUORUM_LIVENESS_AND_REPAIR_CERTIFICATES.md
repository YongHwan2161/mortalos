...
Status: **CURRENT SOURCE/RUNTIME/TEST COMPLETE-SUITE PASS; POST-RUN DOCS STATIC PASS; EXACT-SHA EXTERNAL; EXTERNAL ISSUER HONESTY/INDEPENDENCE HOLD**
...
| `node --test --test-concurrency=2 test/confidential-journal-v2.test.mjs test/placement-repair-batch.test.mjs test/placement-repair-executor.test.mjs test/placement-repair-schedule.test.mjs` | Exact heavy-group `13/13` PASS in `2,650,963.2847ms`. The 128-cycle signed journal ceiling shares only two test-file workers with batch/executor/schedule. Under that bounded load, batch interruption uses `871,815.9221/1,200,000ms`, executor `1,211,615.595/1,500,000ms`, and schedule `780,052.3548/900,000ms`; assertion corpora are unchanged. |
