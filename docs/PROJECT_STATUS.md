...
| Mortality observer | P0 reconciliation locally verified | The lineage supplies its recognized head through a private constructor capability, blocks reentrant mutation, owns every declared carrier, recursively discovers target-tuple bodies and exact tagged signatures, bounds body occurrences and aggregate canonical bytes, exhaustively exact-caches signature remapping, and filters projected usable signers through sign-once commitments. Authenticated multi-body evidence, payload uncertainty, incomplete inventory, forks, and fixed work-limit overflow remain unclassified. Runtime/crypto drift—including after a fork—and malformed declared carriers abort. Death requires exact irreversibility plus explicit completeness. The honest Proxy-free producer, carrier placement, completeness, global usability, and hidden-copy assumptions remain outside cryptographic proof. |
| Portable adversarial corpus | v6 candidate | Corpus v6 adds candidate-count/canonical-byte overflow and post-fork runtime integrity to explicit incomplete-versus-complete mortality evidence, strict points, trust boundaries, same-body completion, sign-once-aware equivocation, missing-membership-payload uncertainty, 15/15 named negatives, ten boundary outcomes, and 10,000 fixed-seed rejects. Node/browser-target/actual-Chromium equality is required on the exact publication head. H3A additionally verifies actual-browser SAB rejection under cross-origin isolation. |
| Node/Chromium equivalence | Node/browser-target local PASS; actual browser pending | The committed v6 result, direct Node 24, and isolated browser-target realm must be byte-identical on this candidate. Local Playwright Chromium installation returned a zero-byte/truncated CDN archive even after a workspace-cache and elevated retry, so the final immutable remote head must supply actual-Chromium evidence; any code change invalidates the prior run. |
...
- conformance tests: 74/74 PASS locally on this candidate, including 26/26 focused mortality cases;
...
- portable boundary probes: 10/10 expected outcomes in Node and the isolated browser-target, adding deterministic candidate-count and candidate-canonical-byte overflow;
- portable fixed-seed cases: 10,000/10,000 expected rejects from seed `1297044052`;
- committed portable v6 result, direct Node 24, and isolated browser-target realm: byte-identical; 10,000/10,000 adversarial rejects; actual Chromium remains part of the exact-head remote gate;
- trusted-core coverage: 96.17% line, 92.98% branch, and 95.20% function across 73 tests, including `src/primordials.mjs`; `src/lineage.mjs` branch coverage is 93.05% and the aggregate branch floor is 90%;
...
- clean locked installation and full `npm test`: PASS; conformance 74/74, governance 30/30, mortality 26/26, Lab 5/5, static Lab build, portable v6, singleton, and H2 v4 all pass;
...
### Closed in P0 candidate — candidate work and signature remapping were not fully bounded

Independent review showed that a 19,738-byte sidecar could contain 129 target occurrences and that a 71,441-byte, 64-layer target could amplify retained canonical work past 4 MiB; both previously reached a death result. It also showed that the same signature multiset could return either non-death or limit overflow depending on where the first valid signature appeared. The observer now fixes limits of 16 supplied usable IDs, 768 total usable-ID UTF-16 code units, 128 pending records, 4 MiB of owned pending bytes, 128 target occurrences, 4 MiB of aggregate candidate canonical UTF-8 bytes, and 1,152 conservative attacker-proportional signature-verification work units. Remapping is exhaustive and stable; only exact body/domain/signer/signature results are cached. Every usable ID must be one canonical `peer:` identifier. Fixed crypto known-answer tests are constant integrity overhead outside that budget. Exact/+1 regressions prove frozen unclassified overflow, graph atomicity, and normal retry. This trades an availability false-negative under evidence flooding for the safety property that resource pressure or evidence order cannot manufacture a death conclusion.
