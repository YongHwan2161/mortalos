...
| WebKit durable participant parity | Capability-routed candidate | Windows rejects Ed25519; Ubuntu creates a key but fails the protocol-ceiling/full-quorum signing envelope, so both current runtimes are verifier-only | No | No | Engine name or key generation never grants authority; full custody requires the exact bounded sign/verify probe and no raw-key fallback is allowed. |
