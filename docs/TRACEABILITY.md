...
Status: **Normative v0 baseline with resource-bounded portable core, committed H2 v4 evidence, and publication-candidate Chromium CI verification**
...
| `INV-12` Authority availability and state availability are never conflated. | v0 state loss is `state-stalled`; only irreversible below-quorum authority loss with no latent successor in an explicitly complete declared inventory establishes v0 protocol death under controlled assumptions. | Observer-state result; no death message | `T-P4-DEATH-001`, `T-P4-DEATH-BASIS-001`, `T-P4-STATE-STALLED-001`, `T-P6-UI-STATE-001` | P4 |
| `INV-13` Destroying current private keys does not revoke previously created authorization evidence. | Public latent validation authenticates a supplied current quorum. Mortality recursively discovers target-tuple bodies and exact tagged signatures throughout every parsed declared tree, indexes sidecars independently, remaps signatures to eligible roles for each exact body, filters one early usable-key snapshot through sign-once commitments, and refuses unsupported death when evidence is incomplete, equivocating, payload-opaque, or already proves a latent child. | `latent successor / not dead`, `authority_unavailable_not_proven_dead`, `evidence_equivocation`, or conditional `evidence_payload_unavailable` observer result | `T-P4-LATENT-001`, `T-P4-LATENT-COMPLETION-001`, `T-P4-EVIDENCE-COALESCE-001`, `T-P4-EVIDENCE-POISON-001`, `T-P4-EVIDENCE-RECURSIVE-001`, `T-P4-SIGN-ONCE-001`, `T-P4-PAYLOAD-OPAQUE-001`, `T-P5-DELAY-001` | P4 |
...
| `INV-15` One validation operation observes one immutable input basis within its declared profile. | Intrinsic byte metadata and owned snapshots precede semantic use; mortality reads only bounded named fields and indices, treats unknown fields as non-evidence, aborts on known accessors, sparse indices, unsnapshotable/malformed declared carriers, internal uncertainty, or realm/crypto drift, and gives phase two only owned parsed data. Transparent Proxy observer structures remain explicitly outside the v0 profile. | Parse/payload code, input-contract abort, or `E_VALIDATOR_INTERNAL` | `T-P1-SNAPSHOT-001`, `T-P1-SAB-001`, `T-P1-IJSON-001`, `T-P1-IJSON-SLOT-001`, `T-P1-TOTAL-001`, `T-P4-LATENT-TOCTOU-001`, `T-P4-OBS-DATA-001`, `T-P4-CARRIER-UNCERTAINTY-001`, `T-P4-PRIMORDIAL-001`, `T-P4-DEPENDENCY-KAT-001` | P1 |
| `INV-16` Mortality never trusts a caller-selected head or infers completeness from absence. | `Lineage` requires a module-private construction token; `Lineage#evaluateMortality` uses the graph-recognized current head, blocks reentrant mutation, reconstructs possible direct children from raw components, and requires exact irreversibility plus explicit completeness for death; strict siblings record a fork, while noncanonical multi-body evidence remains unclassified without graph mutation. | Observer `forked`, `authority_unavailable_not_proven_dead`, `evidence_equivocation`, `evidence_payload_unavailable`, or conditional mortality result | `T-P4-HEAD-SCOPE-001`, `T-P4-CONSTRUCTOR-001`, `T-P4-PENDING-RAW-001`, `T-P4-PENDING-FORK-001`, `T-P4-REPAIRABLE-FORK-001`, `T-P4-FORK-UNCLASSIFIED-001`, `T-P4-REENTRANCY-001`, `T-P4-DEATH-BASIS-001`, `T-P4-LATE-EVIDENCE-001` | P4 |
...
| `INV-18` Resource pressure cannot turn incomplete mortality analysis into a life or death decision. | The observer applies fixed whole-evaluation limits to usable-ID count and UTF-16 code units, pending records, owned pending bytes, and conservative attacker-proportional signature-verification work; usable IDs must be canonical before keyed lookup, and overflow stops without truncation or graph mutation. | Observer `indeterminate / limit_exceeded` | `T-P4-LIMIT-001`, `T-C1-DIFF-LIMIT-001` | P4 |
...
| One reference validator produces the same first result reproducibly. | The committed expected result, direct Node execution, and isolated browser-target bundle must agree for portable corpus v6; actual Chromium must agree for the exact review head. R1 additionally requires an independently written non-JavaScript verifier over canonical operation/result bytes. |
...
| Empty or incomplete local evidence is misreported as death | `T-P4-DEATH-BASIS-001`, `T-P4-LATE-EVIDENCE-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) — “death requires irreversible loss and an explicitly complete evidence inventory” |
| Exact target bodies/signatures are hidden in nested values or property names | `T-P4-EVIDENCE-RECURSIVE-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) — “mortality discovers exact bodies and signatures throughout declared JSON trees” |
| Realm or dependency replacement changes array/collection/scalar/regex/typed-array/schema/crypto behavior | `T-P4-PRIMORDIAL-001`, `T-P4-DEPENDENCY-KAT-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) descriptor, SHA-IV, `startsWith`, and self-restoring metadata regressions |
| A recorded fork bypasses runtime/dependency integrity | `T-P4-FORK-KAT-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) — “fork mortality results remain behind the runtime-integrity gate”; portable corpus v6 repeats the probe |
| Nested or duplicate target bodies amplify canonicalization/retention | `T-P4-CANDIDATE-WORK-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) — “candidate discovery limits bound duplicate count and recursive canonical retention” with exact/+1, atomicity, and retry |
| Signature placement changes work or sign-once outcome | `T-P4-SIGNATURE-ORDER-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) — “signature remapping is cached, exhaustive, and bounded at 1,152 genuine checks” |
| Declared mortality carrier is accessor-backed, detached, malformed, oversized, or wrong-type | `T-P4-OBS-DATA-001`, `T-P4-CARRIER-UNCERTAINTY-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) whole-observation abort regressions |
| Transparent Proxy lies or evidence is placed in an unknown observer field | Documented residual: the trusted adapter MUST provide Proxy-free containers and place evidence in a documented carrier. R1 replaces the object graph with canonical operation/result bytes; completeness honesty remains a policy assumption. |
...
| Oversized mortality observation causes truncation or resource exhaustion | `T-P4-LIMIT-001`, `T-C1-DIFF-LIMIT-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) exact/+1 resource regressions; portable corpus v6 pending-record, usable-ID-character, candidate-count, and candidate-canonical-byte overflow |
...
| Pending evidence recomposition and recognized-head mortality scope | [`test/lineage.test.mjs`](../test/lineage.test.mjs) and [`test/mortality.test.mjs`](../test/mortality.test.mjs): target-tuple bodies and exact tagged signature values/property names are recursively discovered across parsed declared trees; candidate occurrences/canonical bytes are bounded before retention; declared sidecar roots are indexed independently; signatures are exhaustively remapped and exact-cached per body/domain/signer/signature; malformed declared carriers abort while parseable irrelevant JSON is inert; distinct bodies never cross-union; missing membership payload, incomplete inventory, and equivocation remain unclassified; observable mutation and realm poisoning—including after a fork—cannot alter the owned basis; death requires explicit completeness; strict siblings record a fork | PASS within the trusted Proxy-free adapter boundary |
| Mortality resource exhaustion cannot classify a truncated observation | Fixed limits cover usable-ID count and UTF-16 code units, pending records, owned pending bytes, candidate occurrences, aggregate candidate canonical UTF-8 bytes, and conservative attacker-proportional signature-verification work; focused tests cover every exact/+1 limit and portable corpus v6 includes deterministic pending-record, oversized-ID, candidate-count, and candidate-byte overflow | PASS |
...
| Cross-runtime portable result corpus | Format v6 covers strict points, hostile metadata, explicit incomplete and complete mortality evidence, deterministic outcomes, same-body completion, sign-once-aware equivocation, missing-membership-payload uncertainty, fixed mortality/candidate-limit overflow, post-fork runtime integrity, and strict rejection. The committed fixture, direct Node 24, and isolated browser-target are byte-identical locally with 10,000/10,000 rejects. Actual Chromium must agree on the exact reviewed head; local browser installation was blocked by a zero-byte CDN archive. | Local Node/browser-target PASS; exact-head actual-Chromium gate required |
...
| Fixed reference turnover, mutations, resurrection, explicit-complete mortality, runtime-drift aborts, and clone | `lab/reference-engine.mjs`, `test/lab.test.mjs`, `scripts/verify-lab.mjs` | Unit PASS; exact-head Chromium rerun required |
