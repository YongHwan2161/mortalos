...
| One reference validator produces the same first result reproducibly. | The committed expected result, direct Node execution, and isolated browser-target bundle must agree for portable corpus v5; actual Chromium must agree for the exact review head. R1 additionally requires an independently written non-JavaScript verifier over canonical operation/result bytes. |
...
| Oversized mortality observation causes truncation or resource exhaustion | `T-P4-LIMIT-001`, `T-C1-DIFF-LIMIT-001`; [`test/mortality.test.mjs`](../test/mortality.test.mjs) — “mortality resource limits abort without truncating evidence into death”; portable corpus v5 pending-record and usable-ID-character overflow |
...
| Pending evidence recomposition and recognized-head mortality scope | [`test/lineage.test.mjs`](../test/lineage.test.mjs) and [`test/mortality.test.mjs`](../test/mortality.test.mjs): target-tuple bodies and exact tagged signature values/property names are recursively discovered across parsed declared trees; declared sidecar roots are indexed independently; signatures are remapped per body; malformed declared carriers abort while parseable irrelevant JSON is inert; distinct bodies never cross-union; missing membership payload, incomplete inventory, and equivocation remain unclassified; observable mutation and realm poisoning cannot alter the owned basis; death requires explicit completeness; strict siblings record a fork | PASS within the trusted Proxy-free adapter boundary |
| Mortality resource exhaustion cannot classify a truncated observation | Fixed limits cover usable-ID count and UTF-16 code units, pending records, owned pending bytes, and conservative attacker-proportional signature-verification work; focused tests cover every limit and portable corpus v5 includes deterministic pending-record and oversized-ID overflow | PASS |
...
| Cross-runtime portable result corpus | Format v5 covers strict points, hostile metadata, explicit incomplete and complete mortality evidence, deterministic outcomes, same-body completion, sign-once-aware equivocation, missing-membership-payload uncertainty, fixed mortality-limit overflow, and strict rejection. The committed fixture, direct Node 24, and isolated browser-target are byte-identical locally with 10,000/10,000 rejects. Actual Chromium must agree on the exact reviewed head; local browser installation was blocked by a zero-byte CDN archive. | Local Node/browser-target PASS; exact-head actual-Chromium gate required |
