...
| Reopened S2/S4 capability ownership | `lab/storage/durable-store.mjs`, `lab/participant/durable-quorum-endpoint.mjs`, `src/primordials.mjs`, `src/state/recovery.mjs`, `src/confidential/recovery.mjs`, `security/async-entrypoints.json` | Public/prototype method replacement is inert; transitive input ownership marker precedes first await; commit readback must equal candidate |
| Generated protocol profile and relay chunk plane | `protocol/profile.v1.json`, `scripts/generate-protocol-profile.mjs`, `src/transport/chunk-data-plane.mjs`, `test/protocol-profile.test.mjs`, `test/transport.test.mjs` | Source/generated equality, exact/max+1 boundaries, and real 1 MiB relay reconstruction PASS |
...
| Browser capability/parity | `scripts/verify-browser-capabilities.mjs`, `scripts/verify-browser-parity.mjs` | Chromium/Firefox full candidate paths PASS; WebKit portable verifier PASS and signer fails closed on Ed25519 `NotSupportedError` |
