...
| S5 SDK/CLI public boundary | `sdk/index.mjs`, `sdk/continuity.mjs`, `cli/mortalos.mjs`, `cli/node-authority.mjs`, `test/sdk.test.mjs`, `test/continuity.test.mjs`, `scripts/verify-sdk-package.mjs` | Default authority-free export allowlist; explicit create/inspect/handoff/recover/continue capability API; clean packed consumer completes the real-file CLI flow without repository-relative imports; exchanged artifacts contain no private key; two conflicting Node processes have exactly one signer under the authority-file lock |
| P0 real-file continuity vertical | `src/continuity.mjs`, `lab/product-continuity.mjs`, `test/continuity.test.mjs`, `test/continuity-node-endpoint.mjs`, `scripts/verify-continuity-chromium.mjs` | Runtime file, B acceptance, actual A process/context exit, one corrupt copy plus two valid copies, exact byte recovery, and B sequence-3 commit PASS in separate Node processes and actual built-Lab Chromium; one copy, stale lineage, wrong authority, and valid fork reject |
| S6/S8 Continuity Capsule custody | `src/capsule.mjs`, `src/custody.mjs`, `test/capsule.test.mjs`, `test/custody.test.mjs`, `test/continuity.test.mjs` | Cross-process verification, post-state membership lineage, tamper rejection, one corrupt/lost copy recovery, fork rejection, and no private-key marker PASS; product Capsule resource bytes are not claimed confidential |
