...
| Direct WebRTC carrier | `lab/transport/webrtc-peer.mjs`, `src/transport/protocol.mjs`, `test/webrtc-transport.test.mjs`, `scripts/verify-p2p-placement-chromium.mjs` | Browser-platform adapter stays outside portable `src/`; canonical bounded signaling, ordered binary frames, no implicit fallback, owned publish bytes, immutable internal frame snapshots, detached frozen publish/fetch/subscriber results, accessor/Proxy/SharedArrayBuffer rejection, send-before-local-commit failure atomicity with real retry, exact artifact revalidation, and zero origin/relay requests after the cut |
