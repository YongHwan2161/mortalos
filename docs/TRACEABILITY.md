...
| Reopened S2/S4 capability ownership | `lab/storage/durable-store.mjs`, `lab/participant/durable-quorum-endpoint.mjs`, `lab/participant/webcrypto-key-store.mjs`, `src/primordials.mjs`, `src/state/recovery.mjs`, `src/confidential/recovery.mjs`, `security/async-entrypoints.json` | Public/prototype method replacement is inert; endpoint/store diagnostics and signer/WebCrypto facades receive no private key; public raw writes reject; verifier-owned entrypoint inventory, tokenized pre-await marker, post-await borrowed-identifier audit, and hostile negative tests pass; branded activation readback must equal candidate |
