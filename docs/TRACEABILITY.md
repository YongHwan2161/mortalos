...
| Browser capability/parity | `scripts/browser-capability-probe.mjs`, `scripts/verify-browser-capabilities.mjs`, `scripts/verify-browser-parity.mjs` | Chromium/Firefox full candidate paths PASS; WebKit runs full custody when native Ed25519 exists and otherwise takes an explicit verifier-only fail-closed route |
