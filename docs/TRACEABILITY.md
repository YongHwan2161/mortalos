...
| Signed bounded resource contract | `protocol/profile.v1.json`, `src/crypto.mjs`, `src/resource-contract.mjs`, `sdk/resource-contract.mjs`, `test/resource-contract.test.mjs`, `scripts/verify-sdk-package.mjs`, `docs/RESOURCE_CONTRACT.md` | Finite storage/bandwidth/compute offer, mutual contained lease, dual-signed cumulative usage, unilateral revocation, exact role domains, single-use equivocation halt, stale/fork/replay rejection, explicit time, generated limits/max+1, hostile input, Node/browser-target/portable/packed-consumer PASS required; data-plane delivery and provider independence remain HOLD |
