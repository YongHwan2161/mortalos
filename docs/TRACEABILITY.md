...
| `INV-18` Resource pressure cannot turn incomplete mortality analysis into a life or death decision. | The observer applies fixed whole-evaluation limits to usable IDs, pending records, owned pending bytes, and conservative signature-verification work; overflow stops without truncation or graph mutation. | Observer `indeterminate / limit_exceeded` | `T-P4-LIMIT-001`, `T-C1-DIFF-LIMIT-001` | P4 |
...
| Mortality resource exhaustion cannot classify a truncated observation | Fixed limits cover usable IDs, pending records, owned pending bytes, and conservative signature-verification work; focused tests cover every limit and portable corpus v4 includes deterministic pending-record overflow | PASS |
