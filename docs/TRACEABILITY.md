...
| S5 SDK/CLI public boundary | `sdk/index.mjs`, `cli/mortalos.mjs`, `test/sdk.test.mjs` | Exact authority-free export allowlist and packed-file allowlist PASS |
| S6/S8 Continuity Capsule custody | `src/capsule.mjs`, `src/custody.mjs`, `test/capsule.test.mjs`, `test/custody.test.mjs` | Cross-process verification, tamper rejection, one corrupt/lost copy recovery, fork rejection, and no private-key marker PASS |
...
| Browser capability/parity | `scripts/browser-capability-probe.mjs`, `scripts/verify-browser-capabilities.mjs`, `scripts/verify-browser-parity.mjs` | Chromium/Firefox full candidate paths PASS; WebKit runs full custody only after non-extractable Ed25519 sign/verify succeeds at 1, 1,024, and 65,536 bytes, otherwise it takes an explicit verifier-only fail-closed route |
