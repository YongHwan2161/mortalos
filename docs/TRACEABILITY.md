...
| Reopened S2/S4 capability ownership | `lab/storage/durable-store.mjs`, `lab/participant/durable-quorum-endpoint.mjs`, `lab/participant/webcrypto-key-store.mjs`, `src/primordials.mjs`, `src/state/recovery.mjs`, `src/confidential/recovery.mjs`, `security/async-entrypoints.json` | Public/prototype method replacement is inert; public signer/WebCrypto facades receive no key; scoped exported-async completeness and per-function ownership-before-await are audited; branded activation readback must equal candidate |
| Generated protocol profile and relay chunk plane | `protocol/profile.v1.json`, `scripts/generate-protocol-profile.mjs`, `src/transport/chunk-data-plane.mjs`, `test/protocol-profile.test.mjs`, `test/transport.test.mjs` | Source/generated equality, exact/max+1 boundaries, nested chunk ownership before await, and real 1 MiB relay reconstruction PASS |
