...
| S4 ciphertext-only capture and key custody | `src/confidential/keys.mjs`, `src/confidential/package.mjs`, and `test/confidential-package.test.mjs` | No plaintext marker/digest/raw epoch key/private key; RSA and unwrapped AES private material non-extractable; manifest transition/authority/key-set recomputation rejects local-authority substitution before reservation |
...
| S4 removal and rotation | membership removal plus same-membership authority-equivocation rotation in `test/confidential-package.test.mjs` | Removed `N` custodian denied `N+1`; survivors succeed; fresh authority/AES key and complete rewrap required |
