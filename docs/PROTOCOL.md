...
Programmatic canonicalization MUST accept only I-JSON primitives, dense arrays, and own-data records whose reported direct prototype is either `null` or the realm's intrinsic `Object.prototype`. It MUST reject sparse or extended arrays, custom prototypes, recognized ECMAScript internal-slot or boxed objects, cycles, accessors, symbols, non-enumerable properties, and `undefined`, function, symbol, or bigint values rather than silently eliding or rewriting them. Parser-created null-prototype records carry an internal brand; arbitrary null-prototype inputs are checked with captured, read-only intrinsic-slot probes that reject Date, Map, Set, WeakMap, WeakSet, boxed primitive, RegExp, ArrayBuffer, SharedArrayBuffer, DataView, typed-array, and WeakRef instances after prototype removal. Accessors exposed by the resulting snapshot MUST NOT be invoked. Each container is canonicalized from one own-property descriptor snapshot; repeated aliases reuse that snapshot. Property writability and configurability do not change the JSON value, so frozen data-only containers remain valid.
...
2. no observed pending body satisfies the sign-once-aware conditional-completion rules above without a new signature from a current key assumed lost, no authenticated equivocation is observed, and no completion-capable payload-opaque membership body is the sole remaining obstacle to death classification; and
3. the observer explicitly asserts `latentEvidenceComplete: true`, meaning that every artifact in its declared observation domain has been placed in a documented carrier. Missing or false completeness is never inferred from an empty list and yields `authority_unavailable_not_proven_dead`.
...
Mortality evaluation MUST be an operation on that lineage graph. The lineage supplies its unique current recognized head and acquires only the five named observer fields `usableKeyIds`, `stateAvailable`, `pendingSuccessors`, `authorityLossIrreversible`, and `latentEvidenceComplete` through captured own-property descriptor lookups. It then snapshots explicitly usable current IDs through at most 16 indexed descriptor lookups and a total 768-character budget, validates each as one canonical `peer:` 32-byte identifier, acquires at most 128 dense own-data pending references the same way, and acquires only each record's named `envelopeBytes` and `eventPayloadBytes` fields. “Characters” in this JavaScript v0 reference limit means UTF-16 code units; R1 replaces this implementation-specific unit with canonical byte lengths. The observer never enumerates caller-owned option, array, or carrier containers. Unknown string or Symbol fields are ignored and are not evidence; a truthful complete inventory MUST place every relevant artifact in one of the two declared byte fields. All declared byte fields are copied into owned snapshots before semantic analysis. A known option or carrier accessor, a sparse known array index, a wrong-type/detached/oversized byte source, malformed declared JSON, or an internal validation failure is ambiguous and MUST abort the whole mortality operation as observer uncertainty; value getters and caller-supplied array iterators are not invoked. Parseable irrelevant JSON remains inert.

Before consulting caller values or returning an already-recorded fork, the observer verifies that captured realm and dependency descriptors are intact and runs fixed SHA-256 and RFC 8032 known-answer tests. It repeats that gate after caller acquisition and before parsing, cryptography, latent analysis, or classification so targeted descriptor traps cannot alter the trusted basis. Those constant self-tests are integrity overhead, not attacker-proportional `signature_verifications` units. Phase two reads only owned snapshots, recursively discovers target-tuple bodies and exact tagged signatures throughout the parsed declared trees, indexes parseable sidecars independently, and reconstructs evidence per exact body. Every matching target-tuple occurrence reserves one `candidate_bodies` unit before canonicalization, including canonical duplicates; immediately after canonicalization its UTF-8 length reserves aggregate `candidate_canonical_bytes` before group lookup, retention, hashing, or validation. Signature remapping evaluates the complete stable signature set for every eligible signer without first-match termination and reuses a result only for the exact canonical body, signature domain, signer, and signature. It filters the one global usable-key snapshot by every authenticated same-tuple sign-once commitment—including commitments to semantically invalid bodies—and never trusts carrier labels as signer identity or evidence role. A committed usable key can contribute only to its already signed body; only usable keys with no observed same-tuple commitment count toward a fresh successor. A caller MUST NOT inject a head, accepted result, latent capability, or chosen per-body usability set. Strict valid siblings record a fork; canonical reconstructable siblings may expose the same fork conservatively, while noncanonical multi-body evidence returns unclassified equivocation without graph mutation. When the graph is forked there is no unique recognized head, so mortality MUST remain unclassified under the same runtime-integrity gate.

This snapshot guarantee applies only to honest, Proxy-free ordinary observer records and arrays. JavaScript cannot distinguish a transparent `Proxy` that reports a compatible prototype and descriptors, while enumerating arbitrary caller properties would reopen an unbounded-work path. Proxy-backed observer structures are therefore outside the v0 mortality-proof input profile. R1 MUST replace this residual object-graph boundary with a canonical, bounded, versioned mortality-observation/result byte record and an independently written non-JavaScript verifier. Evidence completeness and absence of hidden key copies remain explicit policy assertions rather than cryptographic facts.
...
| `pending_bytes` | Cumulative successfully owned pending envelope and sidecar bytes | 4,194,304 |
...
One cache-miss cross-body signature check consumes one verification work unit. The cache key MUST bind the exact canonical body, approval-versus-acceptance domain, signer identity/key, and signature; cached truth MUST NOT cross any of those boundaries. Before ordinary or conditional validation of an extracted envelope, the observer conservatively reserves one unit for every listed approval and acceptance that the validator could verify. This reservation may overestimate work but MUST NOT underestimate it. The 1,152-unit ceiling admits one maximum v0 membership body with 32 observed signatures checked against 16 current approval keys plus 16 new acceptance keys (1,024), direct/reconstructed/conditional reservations (96), and one additional full-envelope margin (32). Multiple maximum bodies or flooded signatures may therefore make the observation indeterminate. All limits are evaluated over the complete observation, not independently per carrier or candidate body. Fixed crypto known-answer tests are constant runtime-integrity overhead outside this attacker-proportional budget.
...
This specification fully determines all v0 lifecycle and envelope validity rules. The repository implements a portable reference transition verifier, latent-evidence verifier, and accepted-object graph. The committed result, Node 22, isolated browser-target, and actual Chromium results are required to be byte-identical on every review head; the latest successful exact-head Verify run is the publication evidence. R1 additionally requires an independently written non-JavaScript verifier over canonical operation/result bytes. No implementation-specific transition callback is part of v0 validity.
...
- evaluates mortality only from the graph-recognized current head; acquires bounded named evidence into owned snapshots; verifies runtime/crypto integrity before every result and after caller acquisition; bounds candidate occurrences and aggregate canonical bytes; recursively discovers target-tuple bodies and exact tagged signatures; exhaustively remaps signatures in stable order and exact-caches each body/domain/signer/signature result; runs ordinary acceptance before internal completion analysis; blocks reentrant mutation; treats authenticated multi-body signing as unclassified, lets completion-capable payload opacity block an otherwise unsupported death classification only after authority loss is declared irreversible, requires explicit `latentEvidenceComplete: true` before death, and leaves forks unclassified;
