...
Suite 1 admits `1..16` custodian wraps and at most `3,098,890` plaintext resource
bytes. The complete canonical confidential package is capped at `4,194,304` bytes
so it always fits the normative S3 raw-resource ceiling. The plaintext ceiling is
derived from the worst legal encoding basis: 16 wraps, the maximum epoch and
counter widths, a present prior-receipt digest, and a 64-character transition ID.
At that boundary the package is `4,194,303` bytes and uses 49 encrypted chunks;
resource `3,098,891` and custodian 17 fail before reservation or encryption. The
larger S3 plaintext ceiling does not apply to pre-encryption application bytes,
because ciphertext, metadata, wraps, receipts, and Base64url/JCS expansion must
share the same S3 package budget.
...
4. for authority loss or equivocation, a fresh strict Ed25519 counter-authority key
   ...
   once as an inert own-data record before authorization checks. Both current and
   next custodian arrays are copied by dense own-data index, and every recipient
   descriptor is rebuilt as a frozen three-field record before any asynchronous
   work. Its exact branded authority capability supplies both the approved
   descriptor and every reservation through module-private dispatch. A stateful
   accessor therefore cannot present approved authority `X` and later construct
   authority `Y`; an async callback cannot replace the quorum-approved recipient
   set `X` with a different wrap set `Y`. Its
