...
Status: **CURRENT RUNTIME/TEST/WORKFLOW FULL SUITE PASS; CURRENT DOCS SPEC/LINK/DIFF PASS; EXACT-SHA EXTERNAL; PHYSICAL PROMOTION HOLD**
...
| Complete repository regression | `npm test` | Current frozen runtime/test/workflow PASS in `8,631,790ms` (`143m 51.790s`) through final `verify:s4`; covered files stayed unchanged and related workload count was zero excluding the probe. Docs pass separate spec/link/diff, not a whole-current-tree exact full-suite. The prior `8,076.826s` run is historical; exact-SHA CI remains the publication authority. |
