...
- Rotation captures the successor request once as an inert own-data record.
  The approved descriptor and package reservation resolve through the same exact,
  WeakMap-branded authority capability; caller wrappers, accessors, and repeated
  public `descriptor` or `reserveRange` reads cannot substitute a different
  successor after quorum authorization. Actual Chromium must reject the stateful
  `X`-then-`Y` successor substitution before either authority reserves a counter.
...
failover-local authority rejection, removed-member denial, authority-only rotation, and
