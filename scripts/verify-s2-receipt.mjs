...
const EXPECTED_RECEIPT_DIGEST =
  "sha256:929d13da40c93862e3569d60a4b25e53b2dc334c22586e04f08bcc30e7270b61";
const EXPECTED_SOURCE_COMMIT = "3b4b16e7b0cf73e61259ebb3f65480c69de3244c";
...
const EXPECTED_SCHEMA_DIGEST =
  "sha256:88abec9b1bed5a28752f1f47951e33f4593d247d6f0b63a890aead526242200a";
const EXPECTED_TEST_DIGEST =
  "sha256:2bfec695f2613cacb547356091baa35ea5e93444d69b9258ffb5bfc1cd72a9c1";
...
function assertSemanticEvidence(receipt) {
  ...
  assert.equal(receipt.results.durable_node_tests.passed, 7);
  assert.equal(receipt.results.durable_node_tests.failed, 0);
  ...
  assert.equal(receipt.results.migration.corrupt_v1_aborted, "PASS");
  assert.equal(receipt.results.migration.old_version_retained, true);
  assert.equal(receipt.results.authority_policy.removal_prevents_future_signing, "PASS");
  assert.equal(receipt.results.authority_policy.hard_coded_30_day_lifetime, false);
...
export async function verifyS2Receipt({
  ...
  if (sourceIsCurrentAncestor) {
    ...
    assert.equal(
      ...
      receipt.base_commit,
      "S2 source commit is not a direct child of the recorded base"
    );
