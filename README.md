...
| Confidential lineage-bound P2P placement source | A native 98,317-byte file becomes an S4 package for B and three distinct 2-of-3 ciphertext shards cross direct DataChannels. Fresh receipts and a crash-safe journal gate placement. The journal requires a module-private verified evaluation and a complete three-provider receipt barrier. Brand issuance first snapshots recognized records, dense arrays, and byte views into owned inert data, uses captured collection operations, and rechecks the runtime after hostile acquisition and nested validation; selective array-method, Proxy-array, accessor, and collection poisoning cannot fabricate the branded proof basis. Durable commit then rederives the journal from raw signed evidence and rejects plain/clone/Proxy/accessor, empty/partial, and self-hashed incomplete inputs. Restart loading likewise snapshots filesystem listings, uses contained pointer parsing and generation/fork selection, and checks the runtime before and after I/O, so self-restoring prototype poison cannot select a stale existing pointer; the journal remains unsigned local evidence, not hostile-disk tamper proof. Resource-contract status and proof age share the canonical generation evaluation instant, so historical placement timestamps cannot keep an expired or effectively revoked lease active. Three of four keys from the provider-signed offer's witness roster sign a sequence/predecessor-bound, consumer-window non-response certificate after the challenge crosses WebRTC; no global clock or raw timeout alone qualifies the derived repair plan. This is not provider death, breach, termination, penalty, or settlement evidence. A commits the degraded generation, hands controller custody to B without a key transfer, exits, and B repairs under successor-authorized leases and commits the linked successor. Returned frames are detached and immutable; a failed DataChannel send commits no local frame or dedupe state and remains retryable. The direct carrier uses one private transcript map plus captured WebRTC-local collection, iterator, scheduler, DataChannel, and peer-connection capabilities; its relay artifact-kind allowlist separately uses captured Set membership. Named Node and actual Chromium poison cases require a forbidden `verdict` to produce zero send/local/remote visibility while a `challenge` still crosses both peers exactly once. Rogue consumers, stale prior generations, repeated/skipped generation numbers, realm drift, challenge forks, response forks, sibling generation forks, and candidate chains that omit any supplied authenticated Capsule tip halt. Exact-SHA governance is external evidence; supplied-view currentness is not global currentness; stage promotion, Sybil resistance, arbitrary NAT reachability, and physical independence remain HOLD. |
