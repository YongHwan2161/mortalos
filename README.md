...
- resource-bounded recognized-head mortality that recursively discovers target-tuple bodies and exact Ed25519 signature strings throughout every declared envelope/payload JSON tree; bounds both candidate occurrences and their aggregate canonical bytes; exhaustively remaps and exact-caches signature checks independent of carrier order; filters one explicit usable-key snapshot through sign-once commitments; keeps equivocation, payload uncertainty, and incomplete evidence unclassified; verifies its captured runtime and crypto state before every result, including an existing fork; and returns `indeterminate / limit_exceeded` instead of classifying a truncated evidence set;
...
- a v6 committed result corpus with 15 named negatives, ten reported boundary outcomes, explicit incomplete-versus-complete mortality evidence, same-body completion, payload-unavailability, candidate-work limits, repairable-fork runtime integrity/equivocation, and 10,000 seeded adversarial cases; Node and the isolated browser-target VM exercise every portable case; and
