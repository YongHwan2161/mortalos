...
| S5/S6 product continuity — portable use | The default SDK remains verification-only; `@mortal-os/core/continuity` and the CLI expose create/inspect/handoff/recover/continue through explicit authority capabilities. A canonical Capsule binds lineage plus exact resource bytes, and clean-package Node plus built-Lab Chromium complete A→B recovery and continuation. The visible EN/KO journey now selects a native file up to 128 KiB, moves its file-bearing Capsule over direct WebRTC while HTTP carries only signaling/public evidence, destroys A authority, rejects one corrupt copy, downloads exact bytes in B, and commits sequence 3. The product Capsule is not a confidentiality claim; same-host browser evidence is not independent-host or arbitrary-NAT evidence. |
...
npm run verify:visible-file-continuity
