...
function validateStatePackageInput(input) {
  exactKeys(input, ["format", "operation", "transition_id"], "/input");
  if (input.format !== STATE_PACKAGE_INPUT_FORMAT) {
    fail("E_STATE_PACKAGE_INVALID", "/input/format", String(input.format));
  }
  if (input.operation !== "replace-resource") {
    fail("E_STATE_PACKAGE_INVALID", "/input/operation", String(input.operation));
  }
  if (
    typeof input.transition_id !== "string" ||
    !/^[A-Za-z0-9._-]{1,64}$/u.test(input.transition_id)
  ) {
...
export function createStatePackageInput({
  ...
  validateStatePackageInput(input);
...
export function createStatePackage({
  ...
  validateStatePackageInput(input.value);
  const chunks = [];
  const chunkBytes = [];
  const seenChunks = createSet();
  for (let offset = 0, index = 0; offset < resource.byteLength; offset += STATE_PACKAGE_LIMITS.chunk_bytes, index += 1) {
    ...
    const chunkDigest = statePackageChunkDigest(bytes);
    if (setHas(seenChunks, chunkDigest)) {
      fail("E_STATE_PACKAGE_CHUNK_DUPLICATE", `/manifest/chunks/${index}/digest`, chunkDigest);
    }
    setAdd(seenChunks, chunkDigest);
    chunks.push({
      digest: chunkDigest,
      index,
...
export function verifyStatePackage({
  ...
  validateStatePackageInput(inputDocument.value);
