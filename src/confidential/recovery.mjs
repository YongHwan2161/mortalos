...
export async function rotateConfidentialState({
  ...
  if (rotation.reason === "membership_change") {
    ...
    if (
      membershipChanged ||
      !(priorAuthority instanceof LinearizableCounterAuthority) ||
      priorAuthority.descriptor.authority_id !== current.manifest.authority_id ||
    ...
    try {
      priorState = await priorAuthority.inspect(current.manifest.epoch_id);
    } catch {
      priorStateLost = true;
    }
    if (
