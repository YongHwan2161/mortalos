...
export class LinearizableCounterAuthority {
  ...
  async reserveRange({
    ...
    return this.#store.transact(epochId, async (record) => {
      const active =
        record ??
        {
          epoch,
  ...
  async inspect(epochId) {
    return this.#store.inspect(epochId);
  }
  ...
  async retire(epochId) {
    return this.#store.transact(epochId, async (record) => {
      ...
      return { next: { ...record, retired: true }, value: true };
    });
  }
}
...
export async function observeCounterAuthorityEquivocation({
  ...
  if (!(authority instanceof LinearizableCounterAuthority)) {
    confidentialFail(
      "E_CONFIDENTIAL_COUNTER_AUTHORITY",
      "/counter_authority",
      "instance"
  ...
  if (
    ...
    authority.descriptor.authority_id !== evidence.authority_id
  ) {
    confidentialFail(
  ...
  await authority.retire(evidence.epoch_id);
  const retired = await authority.inspect(evidence.epoch_id);
  if (!retired?.retired) {
    confidentialFail(
      "E_CONFIDENTIAL_COUNTER_EQUIVOCATION",
      "/counter_authority",
