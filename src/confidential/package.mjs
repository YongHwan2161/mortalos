...
function resourcePlaintextParts(resourceBytes) {
  ...
  if (resource.byteLength < 1 || resource.byteLength > 4_194_304) {
    confidentialFail("E_CONFIDENTIAL_LIMIT", "/resource", "1..4194304");
  }
...
function assertMembership(custodians) {
  if (!Array.isArray(custodians) || custodians.length < 1) {
    confidentialFail("E_CONFIDENTIAL_MEMBERSHIP", "/custodians", "nonempty");
  }
  const sorted = [...custodians].sort((left, right) =>
...
export async function createConfidentialPackage({
  ...
  const membership = assertMembership(custodians);
  ...
  const reservation = await authority.reserveRange({
  ...
  if (packageBytes.byteLength > CONFIDENTIAL_LIMITS.package_bytes) {
    confidentialFail("E_CONFIDENTIAL_LIMIT", "/package", "5000000");
  }
...
function validateWraps(wraps, manifest, expectedCustodians) {
  ...
  if (expectedCustodians !== undefined) {
    const expected = assertMembership(expectedCustodians);
    if (
...
export async function decryptConfidentialPackage({
  ...
  try {
    ...
    if (
      ...
      metadata.resource_size > 4_194_304
    ) {
