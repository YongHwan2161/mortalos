import { createSet, freeze, setAdd, setHas } from "./primordials.mjs";

export const REJECTION_CODES = freeze([
  "E_PARSE_INVALID_UTF8",
  "E_PARSE_INVALID_JSON",
  "E_PARSE_DUPLICATE_PROPERTY",
  "E_PARSE_NON_IJSON",
  "E_PARSE_LIMIT_EXCEEDED",
  "E_SCHEMA_WRONG_KIND",
  "E_SCHEMA_INVALID",
  "E_SCHEMA_UNKNOWN_FIELD",
  "E_CANONICAL_MISMATCH",
  "E_ARRAY_NOT_SORTED",
  "E_ARRAY_DUPLICATE_KEY_ID",
  "E_BINARY_ENCODING",
  "E_VERSION_UNSUPPORTED",
  "E_HASH_ALGORITHM_UNSUPPORTED",
  "E_SIGNATURE_ALGORITHM_UNSUPPORTED",
  "E_PUBLIC_KEY_INVALID_POINT",
  "E_PEER_ID_MISMATCH",
  "E_ORGANISM_ID_MISMATCH",
  "E_GENOME_HASH_MISMATCH",
  "E_CURRENT_CUSTODY_HASH_MISMATCH",
  "E_STATE_ROOT_ENCODING",
  "E_STATE_GENOME_MISMATCH",
  "E_STATE_INITIAL_BINDING",
  "E_STATE_INPUT_INVALID",
  "E_STATE_LIMIT_EXCEEDED",
  "E_STATE_PRIOR_ROOT_MISMATCH",
  "E_STATE_NEXT_ROOT_MISMATCH",
  "E_STATE_RECEIPT_MISMATCH",
  "E_STATE_CUSTODY_CHANGED",
  "E_STATE_PACKAGE_INVALID",
  "E_STATE_PACKAGE_LIMIT_EXCEEDED",
  "E_STATE_PACKAGE_DECODING_UNSUPPORTED",
  "E_STATE_PACKAGE_CHUNK_ORDER",
  "E_STATE_PACKAGE_CHUNK_DUPLICATE",
  "E_STATE_PACKAGE_CHUNK_SIZE",
  "E_STATE_PACKAGE_CHUNK_DIGEST_MISMATCH",
  "E_STATE_PACKAGE_RESOURCE_ROOT_MISMATCH",
  "E_STATE_PACKAGE_STALE_ROOT",
  "E_STATE_PACKAGE_INPUT_MISMATCH",
  "E_STATE_PACKAGE_RECEIPT_MISMATCH",
  "E_STATE_UNAVAILABLE",
  "E_STATE_RECOVERY_INTERRUPTED",
  "E_CONFIDENTIAL_FORMAT",
  "E_CONFIDENTIAL_LIMIT",
  "E_CONFIDENTIAL_DECIMAL",
  "E_CONFIDENTIAL_CRYPTO",
  "E_CONFIDENTIAL_KEY",
  "E_CONFIDENTIAL_EPOCH",
  "E_CONFIDENTIAL_IV",
  "E_CONFIDENTIAL_COUNTER_RECEIPT",
  "E_CONFIDENTIAL_COUNTER_STALE",
  "E_CONFIDENTIAL_COUNTER_EXHAUSTED",
  "E_CONFIDENTIAL_COUNTER_AUTHORITY",
  "E_CONFIDENTIAL_WRAP",
  "E_CONFIDENTIAL_MEMBERSHIP",
  "E_CONFIDENTIAL_CHUNK",
  "E_CONFIDENTIAL_AAD",
  "E_CONFIDENTIAL_BINDING",
  "E_CONFIDENTIAL_PACKAGE_ROOT",
  "E_CONFIDENTIAL_RECEIPT",
  "E_CONFIDENTIAL_KEY_UNAVAILABLE",
  "E_CONFIDENTIAL_REJECTED",
  "E_CONFIDENTIAL_INTERRUPTED",
  "E_CONFIDENTIAL_ACTIVATION_STALE",
  "E_CONFIDENTIAL_ROTATION",
  "E_EVENT_PAYLOAD_HASH_ENCODING",
  "E_CUSTODIAN_COUNT_RANGE",
  "E_CUSTODIAN_DUPLICATE_KEY",
  "E_QUORUM_TYPE_UNSUPPORTED",
  "E_QUORUM_THRESHOLD_RANGE",
  "E_QUORUM_NOT_MAJORITY",
  "E_GENESIS_APPROVAL_SET",
  "E_PARENT_REQUIRED",
  "E_PARENT_UNKNOWN",
  "E_PARENT_HASH_MISMATCH",
  "E_SEQUENCE_INVALID_FORMAT",
  "E_SEQUENCE_NOT_NEXT",
  "E_LINEAGE_UNKNOWN",
  "E_REPLAY_STALE",
  "E_EVENT_KIND_UNSUPPORTED",
  "E_EVENT_PAYLOAD_REQUIRED",
  "E_EVENT_PAYLOAD_INVALID",
  "E_EVENT_PAYLOAD_MISMATCH",
  "E_HEARTBEAT_STATE_CHANGED",
  "E_HEARTBEAT_CUSTODY_CHANGED",
  "E_HEARTBEAT_PAYLOAD_NONEMPTY",
  "E_MEMBERSHIP_STATE_CHANGED",
  "E_MEMBERSHIP_CUSTODY_UNCHANGED",
  "E_APPROVAL_SIGNER_INELIGIBLE",
  "E_APPROVAL_DUPLICATE",
  "E_APPROVAL_SIGNATURE_INVALID",
  "E_APPROVAL_INSUFFICIENT_QUORUM",
  "E_ACCEPTANCE_SIGNER_NOT_NEW",
  "E_ACCEPTANCE_MISSING",
  "E_ACCEPTANCE_UNEXPECTED",
  "E_ACCEPTANCE_DUPLICATE",
  "E_ACCEPTANCE_SIGNATURE_INVALID",
  "E_NEXT_QUORUM_ACTIVATION_INSUFFICIENT",
  "E_FORK_DETECTED",
  "E_LINEAGE_ALREADY_FORKED",
  "E_RESOURCE_FORMAT",
  "E_RESOURCE_LIMIT",
  "E_RESOURCE_DECIMAL",
  "E_RESOURCE_IDENTITY",
  "E_RESOURCE_SIGNATURE",
  "E_RESOURCE_TIME",
  "E_RESOURCE_CAPACITY",
  "E_RESOURCE_BINDING",
  "E_RESOURCE_USAGE",
  "E_RESOURCE_REPLAY",
  "E_RESOURCE_EQUIVOCATION",
  "E_RESOURCE_WITNESS",
  "E_RESOURCE_REVOCATION",
  "E_RESOURCE_EXECUTION",
  "E_VALIDATOR_INTERNAL"
]);

const rejectionCodeSet = createSet();
for (let index = 0; index < REJECTION_CODES.length; index += 1) {
  setAdd(rejectionCodeSet, REJECTION_CODES[index]);
}

export function rejection(code, fieldPath = "", deterministicDetail = "") {
  const safeCode = setHas(rejectionCodeSet, code) ? code : "E_VALIDATOR_INTERNAL";
  return freeze({
    status: "reject",
    code: safeCode,
    field_path: fieldPath,
    deterministic_detail: deterministicDetail
  });
}
