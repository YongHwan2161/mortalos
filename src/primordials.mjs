...
export function assertOrdinaryDataRecord(value, label = "record") {
...
export function ownDataProperty(value, property, label = "record") {
  const descriptor = ownDescriptor(value, property);
  if (!descriptor) return { present: false, value: undefined };
  return {
    present: true,
    value: requireDataDescriptor(
      descriptor,
      `${label}.${stringConstructor(property)} must be an own data property`
    )
  };
}

export function ordinaryArrayLength(value, label = "array") {
...
export function snapshotOwnDataRecord(value, label = "record") {
  assertOrdinaryDataRecord(value, label);
