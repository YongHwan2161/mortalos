...
test("10,000 seeded loss, reorder, duplicate, partial, and tamper plans are deterministic and never accept metadata alone", async () => {
  const fixture = packageFixture();
  const digests = fixture.statePackage.manifest.chunks.map(({ digest }) => digest);
  let seed = 0x53335233;
  let falseAcceptances = 0;
  for (let index = 0; index < 10_000; index += 1) {
    seed = (Math.imul(seed, 1664525) + 1013904223) >>> 0;
    const lost = seed % 3;
    const missing = (seed >>> 8) % digests.length;
    const inventories = [0, 1, 2]
      .filter((replica) => replica !== lost)
      .map((replica) => digests.filter((_, chunk) => chunk % 3 !== replica));
    if ((seed & 4) !== 0) inventories[0] = [...inventories[0]].reverse();
    if ((seed & 8) !== 0) inventories[1].push(inventories[1][0]);
    if ((seed & 16) !== 0) {
      inventories[0] = inventories[0].filter((digest) => digest !== digests[missing]);
      inventories[1] = inventories[1].filter((digest) => digest !== digests[missing]);
    }
    const destinationInventory = digests.filter((_, chunk) => chunk !== missing && (seed & 32) !== 0);
    const left = await planStateRecovery({
      destinationInventory,
      manifest: fixture.statePackage.manifest,
      sourceInventories: inventories
    });
    const right = await planStateRecovery({
      destinationInventory,
      manifest: fixture.statePackage.manifest,
      sourceInventories: inventories
    });
    assert.deepEqual(left, right, `schedule ${index}`);
    if (left.status === "available") falseAcceptances += 1;
    assert.ok(["recovery_required", "state_unavailable"].includes(left.status));
    if ((seed & 64) !== 0) {
      assert.notEqual(statePackageChunkDigest(new Uint8Array(fixture.statePackage.chunkBytes[missing]).fill(0)), digests[missing]);
    }
  }
  assert.equal(falseAcceptances, 0);
});
