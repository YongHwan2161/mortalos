...
test("source and base substitutions fail closed", async () => {
  await assert.rejects(
    verifyS2Receipt({ receiptOverride: mutate(["source_commit"], receipt.base_commit) }),
    /source commit mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({ receiptOverride: mutate(["base_commit"], receipt.source_commit) }),
    /base commit mismatch/
  );
});

test("same-cardinality source-path and artifact-digest substitutions fail closed", async () => {
  ...
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(
        ["artifact_digests", "lab/storage/durable-document.mjs"],
        `sha256:${"0".repeat(64)}`
      )
    }),
    /committed S2 receipt digest mismatch|artifact digest mismatch/
  );
});

test("handoff, one-loss, fault, and sign-once result substitutions fail closed", async () => {
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "cold_process_handoffs", "passed"], 99)
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "single_endpoint_loss_repair", "C", "passed"], 99)
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "fault_matrix", "unexpected_outcomes"], 1)
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "sign_once", "conflicting_second_signatures"], 1)
    }),
    /committed S2 receipt digest mismatch/
  );
});

test("corruption, migration, removal, and expiry substitutions fail closed", async () => {
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "corruption_matrix", "partial_journal"], "HOLD")
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "migration", "old_version_retained"], false)
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(
        ["results", "authority_policy", "removal_prevents_future_signing"],
        "HOLD"
      )
    }),
    /committed S2 receipt digest mismatch/
  ...
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["results", "authority_policy", "hard_coded_30_day_lifetime"], true)
    }),
    /committed S2 receipt digest mismatch/
  );
});

test("topology, command, limitation, and review substitutions fail closed", async () => {
  await assert.rejects(
    verifyS2Receipt({ receiptOverride: mutate(["topology_digest"], `sha256:${"0".repeat(64)}`) }),
    /committed S2 receipt digest mismatch/
  );
  ...
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["known_limitations", 0], "A substituted but schema-valid limitation.")
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({ receiptOverride: mutate(["review_snapshot"], "PASS without evidence") }),
    /committed S2 receipt digest mismatch/
  );
});
...
test("validation interval and environment substitutions fail closed", async () => {
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["source_committed_at"], "2026-07-25T12:36:11.000Z")
    }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({ receiptOverride: mutate(["completed_at"], "2026-07-25T12:36:09.000Z") }),
    /committed S2 receipt digest mismatch/
  );
  await assert.rejects(
    verifyS2Receipt({ receiptOverride: mutate(["environment", "timezone"], "UTC") }),
    /committed S2 receipt digest mismatch/
  );
});
...
test("an otherwise unmodeled receipt-byte change fails the frozen digest", async () => {
  await assert.rejects(
    verifyS2Receipt({
      receiptOverride: mutate(["commands", 0, "summary"], "A schema-valid substituted summary.")
    }),
    /committed S2 receipt digest mismatch/
  );
});
