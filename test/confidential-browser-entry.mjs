...
globalThis.__MORTALOS_S4_COUNTER_AUTHORITY__ = Object.freeze({
  ...
  async rotationBoundary(databaseName) {
    ...
    try {
      ...
      const unauthorized = await generateCustodianEncryptionKeyPair(
        randomTagged("mortalos-key:")
      );
      const mutableNextCustodians = [...lostCurrent.custodians];
      let recipientSetMutated = false;
      const lostRotated = await rotatePersistentFixture({
        ...
        fault: async (step) => {
          if (step === "rotation:plaintext-recovered") {
            ...
            recipientSetMutated = true;
          }
        },
        nextAuthority: lostNext.facade,
        nextCustodians: mutableNextCustodians,
        reason: "counter_authority_lost",
      ...
      return {
        ...
        recipient_substitution_rejected:
          recipientSetMutated &&
          lostRotated.package.manifest.wraps.every(
            ({ custodian_id: id }) =>
              id !== unauthorized.descriptor.custodian_id
          ),
